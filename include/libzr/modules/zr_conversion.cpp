...
std::optional<ZprSegment> convertZvrOptChunkToZprOptSegment(const std::optional<ZvrChunk> &zvrChunkOpt,
                                                            const ZvrDimensionProperties &properties) {
    if (!zvrChunkOpt)
        return std::nullopt;
...
ZprSegment convertZvrChunkToZprSegment(const ZvrChunk &zvrChunk, const ZvrDimensionProperties &properties) {
    ...
    for (const auto ts : timestamps) {
        for (auto sy = static_cast<int8_t>(sectionCount - 1); sy >= 0; --sy) {
            ...
            if (renderZprSegmentForSectionSnapshot(ts, sy, cachedSnapshots[ts].at(sy), tileViewDeltas))
                break;
        }
    }
...
bool renderZprSegment(const uint8_t cx, const uint8_t cz, const uint8_t sy, const ZrBlockStatesView &sectionView,
                      ZrBlockStatesView &topDownTileView, ZrBlockStatesView &heightmapTileView, const bool ignoreRoof,
                      const bool ignoreWater) {
    const auto current = topDownTileView.get(cx, cz);
    if (current != 0 && current != 12958 && current != 12959)
        return true;

    for (int8_t cy = CHUNK_SIDELENGTH - 1; cy >= 0; --cy) {
        const auto state = sectionView.getBlockState(cx, cy, cz);
        if (state == 0 || state == 12958 || state == 12959)
            continue;
        ...
        if (ignoreRoof &&
            (state == 79 /* bedrock */
             ...
             || (state >= 5772 && state <= 5779) /* snow layers 1-8 */))
            continue;
...
bool renderZprSegmentForSectionSnapshot(const time_t timestamp, const uint8_t sy, const ZrBlockStatesView &sectionView,
                                        TileViewDeltas &tileViewDeltas) {
    ...
    for (uint8_t cx = 0; cx < CHUNK_SIDELENGTH; ++cx) {
        for (uint8_t cz = 0; cz < CHUNK_SIDELENGTH; ++cz) {
            ...
            finished &= renderZprSegment(cx, cz, sy, sectionView, drainedTopDownTileView,
                                         drainedTopDownHeightmapTileView, false, true);
            finished &=
