...
namespace zvcr {
    ...
    auto ReadHandle::deserializeSegment() -> ReadResult<std::shared_ptr<Segment>> {
        auto segment = std::make_shared<Segment>(ctx.sectionCount);
        ...
        for (size_t sectionIndex = 0; sectionIndex < ctx.sectionCount; ++sectionIndex)
            TRY(deserializePackedDeltaData<SECTION_SIZE_BIOMES>(segment->biomeSections.sections[sectionIndex], biomePaletteTable));
    ...
    auto ReadHandle::deserializeFile() -> ReadResult<File> {
        TRY(validateFilePrefix(filePrefix));

        const auto version = TRY(deserializeVersion(ZVCR3D_LATEST_VERSION));
        const auto dimensionType = TRY(deserializeDimensionType());
        ...
        const auto compressed = std::vector<uint8_t>{data.begin() + static_cast<std::ptrdiff_t>(getOffset()), data.end()};
        ...
        ReadHandle regionReadHandle{std::move(uncompressed), maxDeltas};
        regionReadHandle.ctx = ctx;
    ...
    auto readFile(const fs::path &filepath, const size_t maxDeltas) -> ReadResult<File> {
        ...
        try {
            auto result = handle.deserializeFile();
