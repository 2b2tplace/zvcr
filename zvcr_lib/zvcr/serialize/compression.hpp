...
namespace zvcr::serialize {
    ...
    std::vector<uint8_t> compressData(const Bytes& inputData, const int zstdCompressionLevel, const int zstdCompressionThreads) {
    ...
    template<typename Bytes>
    std::vector<uint8_t> decompressData(const Bytes& compressedData) {
