...
namespace zvcr {
    ...
    void LayerContainer2d::setLayer(const LayerTypeId type, const PackedSnapshot& initialState) {
        setLayer(type, Layer2d(PackedDeltaData { initialState }, type));
    }

    void LayerContainer2d::setLayer(const LayerType layerType, const PackedSnapshot& initialState) {
        setLayer(static_cast<LayerTypeId>(layerType), initialState);
    }

    void LayerContainer2d::setLayer(const LayerTypeId type, const std::vector<PackedSnapshot>& reverseDeltas,
                                    const size_t snapshotLength) {
        setLayer(type, Layer2d(PackedDeltaData { reverseDeltas, snapshotLength }, type));
    }

    void LayerContainer2d::setLayer(const LayerType layerType, const std::vector<PackedSnapshot>& reverseDeltas,
                                    const size_t snapshotLength) {
